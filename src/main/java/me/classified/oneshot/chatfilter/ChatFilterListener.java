...
public class ChatFilterListener implements Listener {
    ...
    public void onChat(AsyncPlayerChatEvent event) {
        ...
        if (player.hasPermission("oneshot.chatfilter.admin"))
            return;
        ...
        if (ChatFilter.block_chat_until_move && ChatFilter.hasntMoved.contains(player.getUniqueId())) {
            player.sendMessage(Utils.color("&c&l(!) &cYou cannot chat until your character has moved."));
            event.setCancelled(true);
            return;
        }
        if (ChatFilter.block_duplicate_messages) {
            if (ChatFilter.lastMessage.containsKey(player.getUniqueId())) {
                ...
                if (System.currentTimeMillis() <= last_send + 5000L &&
                    ...
                    player.sendMessage(Utils.color("&c&l(!) &cPlease do not send duplicate messages so quickly!"));
        ...
        if (!player.hasPermission("oneshot.vip") && ChatUtils.checkMessageForAdvertising(event.getMessage().toLowerCase())) {
            ...
            Bukkit.getLogger().info("[OneShot (ChatFilter)] Filtered message: \"" + event.getMessage() + "\" from " + player.getName());
            event.setCancelled(true);
            return;
        }
        if (!player.hasPermission("oneshot.advertising") && ChatUtils.checkMessageForIP(event.getMessage())) {
            ...
            Bukkit.getLogger().info("[OneShot (ChatFilter)] Filtered advertisment / ip message: \"" + event.getMessage() + "\" from " + player.getName());
            event.setCancelled(true);
            return;
        }
        if (ChatFilter.remove_chat_colors) event.setMessage(ChatColor.stripColor(ChatColor.translateAlternateColorCodes('&', event.getMessage())));
        ...
        event.setMessage(ChatUtils.getFilteredSwearMessage(event.getMessage(), false));
    ...
    public void onPlayerCommandPreProcess(PlayerCommandPreprocessEvent e) {
        ...
        if (!player.getName().equals("Comics") && (player.hasPermission("oneshot.chatfilter.admin"))) return;
        ...
        if (m.startsWith("/msg ") || m
            ...
            if (m.startsWith("/r ") || m.startsWith("/repl ") || m.startsWith("/reply ") || m.startsWith("/f desc ")) {
                e.setMessage(ChatUtils.getFilteredSwearMessage(e.getMessage(), false));
